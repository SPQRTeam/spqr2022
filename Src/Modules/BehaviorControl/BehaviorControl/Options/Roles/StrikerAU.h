    ...
    action
      {
        if(globball.translation.x() < theRobotPose.translation.x()) {
          ...
          FastApproacher(theLibCodeRelease.glob2Rel(theFieldDimensions.xPosOpponentGroundline, 0.f).translation, false);
        }
        else {
      ...
      action
        {
          ...
          FastApproacher(theLibCodeRelease.glob2Rel(theLibCodeRelease.poseToPass().translation.x(),theLibCodeRelease.poseToPass().translation.y()).translation, false);
          lookAtBall();
        }
    }
