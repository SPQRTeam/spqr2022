...
float ContextCoordinator::getTemmateOntheBallValue(Pose2f target, Pose2f robotPos) const
{
    ...
    for(const auto& obs : theObstaclesFieldPercept.obstacles) {
        if(obs.type == ObstaclesFieldPercept::ownPlayer){
            if(theLibCodeRelease.distance(obs.center, theLibCodeRelease.glob2Rel(target.translation.x(), target.translation.y()))    <   theLibCodeRelease.distance(robotPos, target)){
...
void ContextCoordinator::updatePlayingRoleSpace(Role& role)
{
    ...
    else
    {
        ...
        defender_pose = Pose2f(defender_pose.translation.x(), theLibCodeRelease.defenderDynamicY());       //VINCENZO
        ...
        */
        ...
        supporter_pose = theLibCodeRelease.getSupporterPosition(); //VINCENZO
        targets.push_back(supporter_pose);       //VINCENZO

        jolly_pose = Pose2f(theLibCodeRelease.getJollyPosition().x(), theLibCodeRelease.getJollyPosition().y()); // TODO VINCENZO
...
void ContextCoordinator::updateSearchRoleSpace(Role& role)
{
    ...
    else
    {
        ...
        targets.push_back((Pose2f)theLibCodeRelease.defenderPosition);
        targets.push_back((Pose2f)theLibCodeRelease.supporterPosition);
